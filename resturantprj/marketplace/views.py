            ...
            if request.headers.get('x-requested-with') == 'XMLHttpRequest':
                ...
                try:
                    ...
                    try :
                         ...
                         check_cart.save()
                         return JsonResponse({'status':'success' , 'message' : "increase the cart quantiy" , 'cart_counter' : get_cart_counter(request), 'qty' : check_cart.quantity  })
                         
                    except:
                        check_cart = CartModel.objects.create(user = request.user , fooditem=fooditem , quantity=1)
                        return JsonResponse({'status':'success' , 'message' : "Added food the Cart" , 'cart_counter' : get_cart_counter(request) , 'qty' : check_cart.quantity })
        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            ...
            try:
                ...
                try :
                    ...
                    return JsonResponse({'status':'success' , 'message' : "Decrease the cart quantiy" , 'cart_counter' : get_cart_counter(request), 'qty' : check_cart.quantity  })
